/* -------------------------------------------------------------
   Magic Math - Kids' Educational Game Logic (What Comes After)
------------------------------------------------------------- */
...
// Practice Queue for Wrong Answers: stores objects like { target: N, remaining: 3 }
let practiceQueue = [];
...
const idleStatements = [
  ...
  "Let's find the number! 🕵️",
  "What comes next? You got this! ✨",
...
function loadNewQuestion() {
  ...
  if (eligiblePractice.length > 0 && Math.random() < 0.6) {
    ...
    currentNumber = Math.floor(Math.random() * (difficultyLimit - 1)) + 1;
    correctAnswer = currentNumber + 1;
  }
  ...
  announceToScreenReader(`New question: What comes after ${currentNumber}? Select from options.`);
}
...
function handleOptionClick(event, buttonIndex) {
  ...
  if (selectedVal === correctAnswer) {
    ...
    triggerConfetti();
    announceToScreenReader(`Correct! ${correctAnswer} comes after ${currentNumber}.`);
