/* -------------------------------------------------------------
   Magic Math - Kids' Educational Game Logic (Magic Subtraction)
------------------------------------------------------------- */

// State variables
let currentNumber = 0; // The target difference
let correctAnswer = 0; // Same as difference
let numA = 0; // Minuend
let numB = 0; // Subtrahend
let score = 0;
let streak = 0;
let oopsiesCount = 0;
let isFirstTry = true;
...
const idleStatements = [
  ...
  "Count the bright stars that are NOT crossed out! 🌟",
  "Let's take away! You can do it! ✨",
  "Magic subtraction time! ☀️",
  "Which box shows the correct difference? 🎁"
];

const practiceStatements = [
  "Let's practice this subtraction! 🧠🩹",
  "Practice makes perfect! You got it! ✨",
  "Let's try this subtraction again! 💪",
  "We can solve this together! 🌟"
];

const successStatements = [
  "Fantastic taking away! ⭐",
...
const wrongStatements = [
  "Let's try again! Count the clean stars! 💪",
  "Almost! Check your count! 🤔",
  "Give it another go! You can do it! 🌟",
  "Oops! Count the remaining stars once more! 😊"
];
...
function loadNewQuestion() {
  ...
  if (eligiblePractice.length > 0 && Math.random() < 0.6) {
    ...
    currentNumber = practiceItem.target; // This is the difference C
    isPracticeQuestion = true;
    
    // Formulate a subtraction equation: A - B = C
    // Pick B randomly such that A = C + B <= difficultyLimit
    const maxB = Math.max(1, difficultyLimit - currentNumber);
    numB = Math.floor(Math.random() * Math.min(10, maxB)) + 1;
    numA = currentNumber + numB;
  } else {
    // Pick A randomly up to difficultyLimit
    numA = Math.floor(Math.random() * (difficultyLimit - 1)) + 2;
    // Pick B randomly from 1 to A-1 (so difference >= 1)
    numB = Math.floor(Math.random() * (numA - 1)) + 1;
    currentNumber = numA - numB;
  }

  correctAnswer = currentNumber;

  // Render visual panel helpers (only draw if minuend numA <= 20)
  const visualPanel = document.getElementById('visual-panel');
  visualPanel.innerHTML = '';
  
  if (numA <= 20) {
    const groupA = document.createElement('div');
    groupA.className = 'equation-group group-subtraction';
    
    const normalCount = numA - numB;
    
    // Draw normal remaining stars
    for (let i = 0; i < normalCount; i++) {
      groupA.innerHTML += starSVG.replace('{INDEX}', i);
    }
    
    // Draw subtracted/crossed out stars
    for (let i = 0; i < numB; i++) {
      const index = normalCount + i;
      groupA.innerHTML += `
        <div class="subtracted-overlay">
          ${starSVG.replace('{INDEX}', index).replace('counting-svg', 'counting-svg subtracted-item')}
        </div>
      `;
    }
  ...
  announceToScreenReader(`New question: ${numA} minus ${numB}. Select from options.`);
}
...
function generateSmartOptions(correctAns, limit) {
  ...
  // Typical mistake distractors
  addIfValid(correctAns + 1);
  addIfValid(correctAns - 1);
  addIfValid(correctAns + 2);
  addIfValid(numA + numB); // Addition confusion distractor! E.g. 5-2 = 7 instead of 3
  ...
  while (distractors.size < 3 && fallbackCounter < 100) {
    fallbackCounter++;
    const randomDecoy = Math.floor(Math.random() * (limit + 2));
...
function handleOptionClick(event, buttonIndex) {
  ...
  if (selectedVal === correctAnswer) {
    ...
    triggerConfetti();
    announceToScreenReader(`Correct! ${numA} minus ${numB} is ${correctAnswer}.`);
